      ...
      <div className="hero-content">
        ...
        <div className="hero-info">
          ...
          <div className="hero-meta">
            ...
            <span className="hero-badge age">{anime.kind ? anime.kind.toUpperCase() : 'ТВ'}</span>
            <span className="hero-badge year"><Calendar size={16} /> {anime.aired_on ? anime.aired_on.slice(0, 4) : '?'}</span>
            <span className="hero-badge episodes">{anime.episodes || anime.episodes_aired || '?'} ЭПИЗОДОВ</span>
          </div>

          <p className="hero-description">Эпичное аниме, которое завоевало сердца миллионов зрителей. Откройте для себя невероятный мир, полный приключений, интриг и неожиданных поворотов сюжета.</p>
          ...
          return (
            <Link to={`/anime/${anime.id}`} key={anime.id} className="row-card">
              ...
              {anime.episodes_aired > 0 && (
                <div className="row-card-badge">Эп. {anime.episodes_aired}</div>
              )}
              {extraBadge && (
  ...
  return (
    <div className="home-container" style={{padding: '0', gap: '0'}}>
      ...
      <div style={{padding: '2rem 5%', display: 'flex', flexDirection: 'column', gap: '3rem'}}>
        <AnimeRow title="В ВЫСОКОМ КАЧЕСТВЕ" animes={hqAnimes} icon={<Star size={32} color="#ffd700" />} extraBadge="1080p | 4K" />
        <AnimeRow title="НОВИНКИ" animes={latest} icon={<Flame size={32} color="var(--accent-color)" />} />
        <AnimeRow title="ПОПУЛЯРНЫЙ ЭКШЕН" animes={actionAnimes} icon={<Swords size={32} color="var(--accent-color)" />} />
